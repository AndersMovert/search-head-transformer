#!/usr/bin/env python3
"""
Vanilla Transformer Baseline — Byte-Level (V=256)

Standard transformer baseline for comparison with the search head.
Uses a single linear projection from the last hidden state to logits:

    Logits = W @ h[t] + b

Same backbone (Temporal Split Attention), same data, same training schedule.
The only difference is the output head — linear vs learned search.
"""
...
CHECKPOINT_FILE = BASE / "checkpoints" / "best_model_vanilla.pt"
LATEST_CHECKPOINT_FILE = BASE / "checkpoints" / "latest_model_vanilla.pt"
...
class VanillaGPT(nn.Module):
    def __init__(self, vocab_size, n_embd, n_head, n_layer, block_size, local_window):
        super().__init__()
        ...
        # Vanilla linear head: single embedding -> logits
...
def main():
    ...
    model = VanillaGPT(
        ...
        block_size=N, local_window=local_window,
    ).to(DEVICE)

    n_params = sum(p.numel() for p in model.parameters())
    print(f"Model: {n_params:,} params | block_size={N} | local_window={local_window}")
    print(f"Heads: {N_HEAD // 2} recent (last {local_window} tokens) + {N_HEAD // 2} historical")
    print(f"Output head: Linear({D} -> {VOCAB_SIZE})")
