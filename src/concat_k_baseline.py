#!/usr/bin/env python3
"""
Concat-K Baseline — Byte-Level (V=256)

Baseline model for comparison with the search head transformer.
Instead of searching for the best context embedding, this model simply
concatenates the last K=5 embeddings and passes them through the MLP head:

    Logits = MLP([h[t], h[t-1], h[t-2], h[t-3], h[t-4]])

Same backbone (Temporal Split Attention), same data, same training schedule.
The only difference is the output head — concat-K vs learned search.

At 2B chars, this reaches 1.85 BPC vs the search head's 1.50 BPC.
"""
...
N_HEAD = 8
N_LAYER = 8
K = 5              # number of embeddings to concatenate for output head
...
CHECKPOINT_FILE = BASE / "checkpoints" / "best_model_concat_k.pt"
LATEST_CHECKPOINT_FILE = BASE / "checkpoints" / "latest_model_concat_k.pt"
...
class ConcatKGPT(nn.Module):
    def __init__(self, vocab_size, n_embd, n_head, n_layer, block_size, k, mlp_head_hidden, local_window):
        super().__init__()
        ...
        # Concat-K head: takes K concatenated embeddings -> logits
        self.head = nn.Sequential(
            nn.Linear(k * n_embd, mlp_head_hidden, bias=False),
            nn.GELU(),
    ...
    def forward(self, idx, targets=None):
        ...
        # Concatenate K consecutive embeddings for positions K-1 onwards
        k = self.k
        valid_len = T - k + 1

        h_concat = h.unfold(1, k, 1)                          # (B, T-k+1, D, k)
        h_concat = h_concat.permute(0, 1, 3, 2).contiguous()  # (B, T-k+1, k, D)
        h_concat = h_concat.view(B, valid_len, k * h.size(-1)) # (B, T-k+1, k*D)

        logits = self.head(h_concat)

        loss = None
        loss_all = None
        loss_last = None
        if targets is not None:
            valid_targets = targets[:, k - 1:]
        ...
        return logits, loss, loss_all, loss_last
...
def main():
    ...
    parser.add_argument("--k", type=int, default=K, help="Number of embeddings to concatenate")
    ...
    wandb.init(
        ...
        config={
            ...
            "K": k, "W": local_window, "MLP_HEAD_HIDDEN": MLP_HEAD_HIDDEN,
            "HEAD_INPUT": f"{k}*D = {k*D}",
    ...
    model = ConcatKGPT(
        ...
        block_size=N, k=k, mlp_head_hidden=MLP_HEAD_HIDDEN, local_window=local_window,
    ).to(DEVICE)

    n_params = sum(p.numel() for p in model.parameters())
    print(f"Model: {n_params:,} params | block_size={N} | K={k} | local_window={local_window}")
    print(f"Heads: {N_HEAD // 2} recent (last {local_window} tokens) + {N_HEAD // 2} historical")
    print(f"Head input: last {k} embeddings concatenated ({k}*{D} = {k*D})")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        while True:
            ...
            with torch.amp.autocast(DEVICE, enabled=use_amp):
                logits, loss, loss_all, loss_last = model(xb, targets=yb)
